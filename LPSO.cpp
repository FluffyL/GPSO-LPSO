...
void update_vel(int i, double vel_max, int lBest_index){
    for (int j = 0; j < N; j++){
        double vel_value;
        vel_value = W * particle[i].vel[j] + C1 * rand()*(1.0/RAND_MAX) * (particle[i].pBest[j] \
                    - particle[i].pos[j]) + C2 * rand()*(1.0/RAND_MAX) * (particle[lBest_index].pBest[j] - particle[i].pos[j]);
...
void update(double pos_max, double vel_max, int fit_func){
    ...
    for (int j = 0; j < SIZE; j++){
        ...
        if (j == 0) {
            if (particle[j+1].pBest_fitness < particle[j].pBest_fitness){
                if (particle[j+1].pBest_fitness < particle[SIZE-1].pBest_fitness) lBest_index = j+1;
                else lBest_index = SIZE-1;}
            else{
                if (particle[j].pBest_fitness < particle[SIZE-1].pBest_fitness) lBest_index = j;
                else lBest_index = SIZE-1;}
        }
        else if (j == SIZE-1) {
            if (particle[j-1].pBest_fitness < particle[j].pBest_fitness){
                if (particle[j-1].pBest_fitness < particle[0].pBest_fitness){
                    lBest_index = j-1;//j+1
                    }
                else lBest_index = 0;}
            else{
                if (particle[j].pBest_fitness < particle[0].pBest_fitness) lBest_index = j;
                else lBest_index = 0;}
        }
        else {
            if (particle[j+1].pBest_fitness < particle[j].pBest_fitness){
                if (particle[j+1].pBest_fitness < particle[j-1].pBest_fitness) lBest_index = j+1;
                else lBest_index = j-1;}
            else{
                if (particle[j].pBest_fitness < particle[j-1].pBest_fitness) lBest_index = j;
                else lBest_index = j-1;}
        }/*
        lBest_index = j;
        int un = (j+1)%SIZE;
        int dn = (j-1+SIZE)%SIZE;
        if (particle[un].pBest_fitness <= particle[lBest_index].pBest_fitness) lBest_index = un;
        if (particle[dn].pBest_fitness < particle[lBest_index].pBest_fitness) lBest_index = dn;*/
        update_vel(j, vel_max, lBest_index);
